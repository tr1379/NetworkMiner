...
namespace NetworkWrapper
{
    public class WinPCapWrapper : WinPCapNative
    {
        ...
        public static List<Device> FindAllDevs()
        {
            ...
            while (i != IntPtr.Zero && i.ToInt64() != 0)
            {
                device = new Device();
                ...
                if (pcap_if.addresses != IntPtr.Zero && pcap_if.addresses.ToInt64() != 0)
                {
                    pcap_addr = ((pcap_addr)(Marshal.PtrToStructure(pcap_if.addresses, typeof(pcap_addr))));
                    if (pcap_addr.addr != IntPtr.Zero && pcap_addr.addr.ToInt64() != 0)
                    {
                    ...
                    if (pcap_addr.netmask != IntPtr.Zero && pcap_addr.netmask.ToInt64() != 0)
                    {
        ...
        public virtual bool Open(string source, int snaplen, int flags, int read_timeout)
        {
            ...
            if (pcap_t != IntPtr.Zero && pcap_t.ToInt64() != 0)
            {
                m_isopen = true;
